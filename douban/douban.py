...
#  https://github.com/janeczku/calibre-web/tree/c2267b690293bc3932dc2588df1d907a7d365f28
import re
...
import logger
from Metadata import Metadata, MetaRecord, MetaSourceInfo
...
class Douban(Metadata):
    ...
    SNIPPETS_XPATH = "//ul[@class='blockquote-list']//figure"
            ...
            with futures.ThreadPoolExecutor(
                ...
                fut = [
                    executor.submit(self._parse_single_book, book_id,
                                    generic_cover) for book_id in first_id_list
                ]
    ...
    def _parse_single_book(self,
        ...
        if html is None:
            log.error(f"html parse is error")
            return None
        ...
        snippets = html.xpath(self.SNIPPETS_XPATH)
        if snippets and len(snippets) > 0:
            for snippet in snippets:    
                snippetText = snippet.text
                snippetText = snippetText.replace("\n            ", "")
                snippetText = snippetText.replace(" (", "")
                match.snippets.append(snippetText)
        ...
        for element in info:
            ...
            if self.AUTHORS_PATTERN.search(text):
                ...
                match.subTitle = f'{element.tail.strip()}'
