...
context = None
page = None
async def catchNovel(playwright, url, nextPagePre):
...
async def readOneNovel(bookTitle, url, mode="complete", nextPagePre = None):
    oldTitle = ""
    index = 1
    # 覆盖写
    writeMode = 'w' 
    ...
    async with async_playwright() as playwright:
        with open(bookTitle + '.txt', writeMode, encoding='utf-8') as f:
            try:
                title, contents, next_url = await catchNovel(playwright, url, nextPagePre)
                while(1):
                    if len(title) > 0:
                    ...
                    time.sleep(0.3)
                    title, contents, next_url = await catchNovel(playwright, next_url, nextPagePre)
...
novelList=[
{
    "url":"https://www.huxixs.com/45_45161/25671962.html",
    "bookTitle":"混迹华娱的我没有格局",
    "mode":"new",
    "nextPagePreUrl":"https://www.huxixs.com",
}
]
