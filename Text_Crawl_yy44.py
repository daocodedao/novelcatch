...
async def catchNovel(playwright, url):
    ...
    # //*[@id="sticky-parent"]/div[2]/div[3]
    titleNode = await page.query_selector('//*[@id="content"]/h1')
    title = await titleNode.text_content()
    title = convert(title, 'zh-cn')
    # //*[@id="content"]/div[5]
    contentNode = await page.query_selector('//*[@id="content"]/div[5]')
    contents = await contentNode.text_content()
    contents = convert(contents, 'zh-cn')

    # //*[@id="mm-5"]/div[2]/div/ul/li[2]/a
    # /html/body/footer/div/a[4]
    # /html/body/section[3]/div/div[1]/ul/li[2]/a[3]
    nextNode = await page.query_selector('//html/body/footer/div/a[4]')
    next_url = await nextNode.get_attribute("href")  #定义text变量接收a标签底下的href属性

    # https://vv44.net/
    next_url = "https://vv44.net/" + next_url
