...
async def catchNovel(playwright, url):
    ...
    titleNode = await page.query_selector('//*[@class="readTitle"]')
    title = ""
    try:
        title = await titleNode.text_content()
        title = convert(title, 'zh-cn')
    except Exception as e:
        print(f"catch title exception {e}")

    contentNode = await page.query_selector('//*[@class="panel-body"]')
    try:
        contents = await contentNode.text_content()
        contents = convert(contents, 'zh-cn')
    except Exception as e:
        print(f"catch contents exception {e}")
...
novelList=[
{
    "url":"https://www.xiaoshuo.com.tw/3683/",
    "bookTitle":"光影文娱",
    "mode":"new"
}
]
